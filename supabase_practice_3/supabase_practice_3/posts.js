...
let cardFeed = document.getElementById("cardfeed");
...
console.log("usersData:", usersData);
            ...
            usersData.forEach((user) => {
              if (!user.userId) {
                console.warn("User missing userId property:", user);
              } else {
            ...
            if (!myId || !myId.userId) {
              console.error("Invalid or missing currentUser in localStorage:", myId);
              return;
            }
            ...
            postData.forEach((post) => {
              ...
              if (!currentUser) {
                console.warn(`Post with ID ${post.id} references a missing userId: ${post.userId}`);
                return; // Skip this post if the user is not found
              }
              console.log(currentUser);
          ...
          <div class="d-flex align-items-center gap-3">
            <img src="${currentUser.profileImage}" alt="" class="rounded-circle" width="40" height="40">
            <h6 class="fw-semibold mb-0 fs-4">${currentUser.name}</h6>
            <span class="fs-2" id="postTime"><span class="p-1 bg-light rounded-circle d-inline-block"></span> ${new Date(post.created_at).toLocaleString()}</span>
            ${
              myPost
              ? `
              <button class="btn btn-danger ms-auto" onclick="deleteMyPost(${post.id})">Delete</button>`
              :  ""
            }
          </div>
          ...
          <img src="${post.imageURL}" alt="" class="img-fluid rounded-4 w-100 object-fit-cover" style="height: 360px;">
